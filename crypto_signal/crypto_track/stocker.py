...
class Prophet():
    ...
    def __init__(self, df_history, value_title='value'):
        ...
        self.most_recent_value = float(
            self.df_history.iloc[len(self.df_history) - 1, self.df_history.columns.get_loc('y')])
    ...
    def plot_chart(self, start_date=None, end_date=None, stats=['y'], plot_type='basic'):
        ...
        for i, stat in enumerate(stats):
            ...
            print('Current {} = {:.2f} on {}.\n'.format(stat, self.df_history.iloc[
                len(self.df_history) - 1, self.df_history.columns.get_loc(stat)], self.max_date.date()))
            ...
            if plot_type == 'pct':
                ...
                plt.title('%s History' % self.value_title);
    ...
    def changepoint_prior_analysis(self, changepoint_priors=[0.001, 0.05, 0.1, 0.2], colors=['b', 'r', 'grey', 'gold']):
        ...
        train = self.df_history[
            (self.df_history['Date'] > (max(self.df_history['Date']) - pd.DateOffset(years=self.training_years)))]
    ...
    def create_prophet_model(self, days=0, resample=False):
        ...
        if days > 0:
            ...
            print('Predicted Value on {} = ${:.2f}'.format(
                future.iloc[len(future) - 1, future.columns.get_loc('ds')].date(),
                future.iloc[len(future) - 1, future.columns.get_loc('yhat')]))
    ...
    def view_prediction(self, start_date=None, end_date=None, include_plot=True):
        ...
        train = self.df_history[(self.df_history['Date'] < start_date) &
                                (self.df_history['Date'] > (
                                        start_date.date() - pd.DateOffset(years=self.training_years)))]
        ...
        for i in test.index:
            if (test.iloc[i, test.columns.get_loc('y')] < test.iloc[i, test.columns.get_loc('yhat_upper')]) & (
                    test.iloc[i, test.columns.get_loc('y')] > test.iloc[i, test.columns.get_loc('yhat_lower')]):
        ...
        if include_plot:
            ...
            print('\nPredicted value on {} = ${:.2f}.'.format(max(future['ds']).date(), future.iloc[
                len(future) - 1, future.columns.get_loc('yhat')]))
            print('Actual value on    {} = ${:.2f}.\n'.format(max(test['ds']).date(),
                                                              test.iloc[len(test) - 1, test.columns.get_loc('y')]))
    ...
    def changepoint_date_analysis(self, search=None):
        ...
        if not search:
            ...
            plt.xlabel('Date');
            plt.ylabel(self.value_title);
            plt.title('%s with Changepoints' % self.value_title)
            plt.show()
        ...
        if search:
            ...
            plt.title('%s Values and Search Frequency for %s' % (self.value_title, search))
            plt.show()
    ...
    def predict_future(self, days=30):
        ...
        plt.xlabel('Date');
        plt.title('Predictions for %s' % self.value_title);
        plt.show()
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares=None):
        ...
        if nshares:
            ...
            for i, correct in enumerate(test_pred_increase['correct']):
                ...
                if correct == 1:
                    prediction_profit.append(
                        nshares * test_pred_increase.iloc[i, test_pred_increase.columns.get_loc('real_diff')])
            ...
            print('The Buy and Hold strategy profit =         ${:.2f}.'.format(
                float(test.iloc[len(test) - 1, test.columns.get_loc('hold_profit')])))
